...
module.exports = async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        modal.addComponents(
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId('user_input').setLabel("Target User (@Mention or ID)").setStyle(TextInputStyle.Short).setRequired(true)),
    ...
    if (interaction.isModalSubmit()) {
        ...
        economyManager.ensureWallet(targetMember.id);
        
        if (action === 'give') db.prepare(`UPDATE wallets SET balance = balance + ? WHERE user_id = ?`).run(amount, targetMember.id);
        else if (action === 'remove') db.prepare(`UPDATE wallets SET balance = balance - ? WHERE user_id = ?`).run(amount, targetMember.id);
        else if (action === 'set') db.prepare(`UPDATE wallets SET balance = ? WHERE user_id = ?`).run(amount, targetMember.id);

        const newWallet = economyManager.getWallet(targetMember.id);
