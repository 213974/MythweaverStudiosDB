...
module.exports = {
    async execute(interaction, userClanData, permissions) {
        if (!permissions.isOwner && !permissions.isVice && !permissions.isOfficer) {
            return interaction.reply({ content: 'You do not have the required authority to kick members.', flags: 64 });
        }

        const targetUserToKick = interaction.options.getUser('user');
        const reason = interaction.options.getString('reason') || 'No reason provided.';

        if (targetUserToKick.id === interaction.user.id) return interaction.reply({ content: "You cannot kick yourself.", flags: 64 });
        ...
        if (!targetUserAuth) return interaction.reply({ content: 'Could not verify the target member\'s authority.', flags: 64 });

        if (targetUserAuth === 'Owner') return interaction.reply({ content: "The Owner cannot be kicked.", flags: 64 });
        if (targetUserAuth === 'Vice Guild Master' && !permissions.isOwner) return interaction.reply({ content: 'Only the Owner can kick a Vice Guild Master.', flags: 64 });
        if (targetUserAuth === 'Officer' && !permissions.isOwner && !permissions.isVice) return interaction.reply({ content: 'Only the Owner or a Vice Guild Master can kick an Officer.', flags: 64 });
        ...
        if (removeResult.success) {
            ...
            return interaction.reply({ content: `Failed to kick member: ${removeResult.message}`, flags: 64 });
        }
    }
};